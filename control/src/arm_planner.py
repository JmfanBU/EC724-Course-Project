...
class Planner(object):
    ...
    def featurize(self, waypts):
        ...
        for index in range(0, len(waypts)-1):
            ...
            elif self.task == BOTH_TASK:
                features[2][index] = self.human_features(waypts[index+1], waypts[index])
                features[1][index] = self.table_features(waypts[index+1])
        return features
    ...
    def human_cost(self, waypt):
        ...
        input trajectory, output scalar cost
        """
        prev_waypt = waypt[0:2]
        curr_waypt = waypt[2:4]
    ...
    def trajOpt(self, start, goal):
        ...
        for t in range(1, self.num_waypts_plan):
            if self.task == TABLE_TASK:
                prob.AddCost(self.table_cost, [(t, j) for j in range(2)], "table%i" %t)
            elif self.task == HUMAN_TASK:
                prob.AddCost(self.human_cost, [(t - 1, j) for j in range(2)] +[(t, j) for j in range(2)],"human%i" % t)
        ...
        for t in range(1, self.num_waypts_plan - 1):
            prob.AddConstraint(self.table_constraint, [(t, j) for j in range(2)], "INEQ", "up%i" %t)
    ...
    def learnWeights(self):
        ...
        if waypts_deform.tolist() is not None:
            ...
            update = Phi_p - Phi
            if update.shape[0] == 2:
                curr_weight = self.weights[self.task] - update_gain*update[1]
                if curr_weight > max_weight:
                    curr_weight = max_weight
                elif curr_weight < 0.0:
                    curr_weight = 0.0
                self.weights[self.task] = curr_weight
            else:
                curr_weight = np.array(self.weights) - update_gain * update[1:3]
                for w in range(curr_weight.shape[0]):
                    if curr_weight[w] > max_weight:
                        curr_weight[w] = max_weight
                    elif curr_weight[w] < 0.0:
                        curr_weight[w] = 0.0
                self.weights = curr_weight.tolist()

            return self.weights, waypts_deform
    ...
    def Correction(self, start, goal):
        ...
        while gap > self.eps:
            ...
            weights, waypts_deform = copy.deepcopy(self.learnWeights())
            times += 1
            print weights, times
            ...
            if self.task == BOTH_TASK:
                prev_Phi = np.array([prev_features[0], sum(prev_features[1]), sum(prev_features[2])])
                curr_Phi = np.array([curr_features[0], sum(curr_features[1]), sum(curr_features[2])])
                opt_Phi = np.array([opt_features[0], sum(opt_features[1]), sum(opt_features[2])])
            else:
            ...
            traj_list.append(copy.deepcopy(waypts_deform))
        traj_list.append(self.waypts_plan.tolist())
        return times, weight_list, traj_list
...
if __name__ == '__main__':
    ...
    for traj in traj_list:
        cur_waypts = []
        for q in traj:
            cur_waypts.append(experiment.robotToCartesian(q).tolist())
        point_list.append(cur_waypts)
    data = {"waypts": point_list,
            "weights": weight_list}
    with open('/home/jiameng/catkin_AIM/src/dvrk_env/dvrk_gazebo_control/src/EC724-Course-Project/data_table_feature.json', 'w') as fp:
        json.dump(data, fp)
